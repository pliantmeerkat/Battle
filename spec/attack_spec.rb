...
describe Attack do
  ...
  context 'Feature 1 - Attack Types' do
    before(:each) do
      allow(damage_inst).to receive(:damage_blunt)
      allow(damage_inst).to receive(:damage_pierce)
      allow(damage_inst).to receive(:damage_default)
      allow(damage_inst).to receive(:damage_return)
